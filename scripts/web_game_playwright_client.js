...
async function doChoreography(page, canvas, steps) {
  for (const step of steps) {
    ...
    if (step.click_selector) {
      await page.click(step.click_selector, { timeout: step.timeout_ms || 5000 });
    }
    ...
    for (const button of buttons) {
      if (button === "left_mouse_button" || button === "right_mouse_button") {
        const bbox = canvas ? await canvas.boundingBox() : null;
        const x = typeof step.mouse_x === "number" ? step.mouse_x : bbox ? bbox.width / 2 : 0;
        const y = typeof step.mouse_y === "number" ? step.mouse_y : bbox ? bbox.height / 2 : 0;
        await page.mouse.move((bbox ? bbox.x : 0) + x, (bbox ? bbox.y : 0) + y);
